...
git remote add origin https://github.com/sixeyed/eshop.git
```
...
[You can check on the workflows here](https://github.com/sixeyed/eshop/actions). It make take a minute or so before the actions start. The workflows use secrets from your repo to connect to Azure. Then they:
...
[Check on the GitHub actions](https://github.com/sixeyed/eshop/actions) to see when the _Deploy to AKS_ workflow has completed.
...
```
az aks get-credentials --resource-group eshop --name eshop-aks
```
...
```
az aks browse --resource-group eshop --name eshop-aks
```
